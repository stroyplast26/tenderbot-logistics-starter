...
from lead_factory.megion_public_permits import MEGION_CSV_HEADERS
...
def megion_workspace(tmp_path):
    ...
    row[7:9] = ["76.10", "61.03"]
    row[10:13] = ["ДЕМО: склад вымышленного объекта", "DEMO-PERMIT-999", "30.07.2026"]
...
def test_megion_dossier_uses_publication_not_import_date_for_freshness(megion_workspace):
    ...
    assert dossier["object"]["title"] == "ДЕМО: склад вымышленного объекта"
    ...
    assert signal["public_fields"]["developer_name"] == "ООО «Синтетический застройщик»"
