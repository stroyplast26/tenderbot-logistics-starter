...
Контроллер `source-discovery-control-v5` связывает batch с хешем точного
канонического пути Source Lab. Копия или перенос controller/Source Lab в другой
каталог не может закрыть исходный batch.

Для новых TenderPlan attempts нужна отдельно подготовленная controller schema 5.
`check` и `run-one` возвращают `BLOCKED_SCHEMA_PREPARATION_REQUIRED`, если
контроллер ещё не создан или использует прежнюю схему. Они не создают и не обновляют схему.
Явная локальная подготовка запускается из выбранной оператором копии программы:

```powershell
.\python_runtime.bat -B scripts/run_source_discovery_once.py prepare-tenderplan-bindings --confirm-local-prepare
```

Команда использует тот же фиксированный controller path. Она атомарно создаёт
новую schema 5 либо расширяет только точно распознанную полную прежнюю schema 4;
повтор на исправной schema 5 не меняет данные. Неизвестные, частичные схемы и
старый формат только с таблицей attempts отклоняются. При RUNNING подготовка
запрещена. Существующие attempts, их WIP и UNCERTAIN остаются прежними: история
не переносится, не закрывается и не дополняется догадками о native receipt.
Новая пустая база не заменяет выбранную рабочую историю и её ограничения.
Сначала оператор должен подтвердить правильную пару программы и state.

Каждый новый TenderPlan attempt требует receipt-связь: его native run ID
однозначно выводится из attempt ID. После native результата controller читает
существующую очередь без расшифровки и сверяет canonical store/path digests,
run ID, receipt/event hashes, текущие query policy/request hashes и упорядоченный
список item IDs. Связь и успешное завершение сохраняются одной транзакцией;
нулевой результат также требует точный native receipt. Потеря или подмена
хранилища и ошибка проверки не создают новую историю и не дают успешный результат.
Сбой оставляет RUNNING/UNCERTAIN для штатной сверки, без автоматического повтора.

`status` показывает `NATIVE_RECEIPT_VERIFIED_AT_FINALIZATION`: receipt был проверен
при завершении, но это не обещание текущей доступности/срока просмотра карточек.
Старые TenderPlan attempts не получают такую отметку. Yandex schema 4, review и
closure продолжают работать по прежним правилам. Native schema, batch closure,
CRM и внешние разрешения эта подготовка не меняет. Проверки этой поставки
выполнены только на временных синтетических базах; рабочая миграция не выполнялась.
