...
14. `InboundMailObservation`.

**MDOS-ARC-003.** В exact inbound mailbox/portal scope нативная почта Bitrix владеет
CRM projection, а `InboundMailObservation` читает IMAP и Bitrix Activities только
через read-only adapters и пишет только локальный reconciliation ledger. Этот context
не входит в `CRMProjectionAndWork` как writer и не имеет repair/outbound capability.
...
**MDOS-EVT-005.** Mail projection считается reconciled только после неизменного
singleton candidate в двух poll на интервале не менее пяти минут. Смена candidate
сбрасывает окно; отсутствие до 90 минут либо ambiguity создают local review, но не
внешний corrective event. Observation, достигшая 90-минутного deadline, позднее не
может автоматически стать reconciled.
...
**MDOS-STO-006.** Observer ledger физически и логически отделён от Bitrix projection.
Он хранит только минимальные hashes/IDs, cursor, candidate digest, stability/review и
quarantine evidence; observer не создаёт и не изменяет Lead, Todo, Timeline, Activity,
task, Company, Contact или Deal.
...
**MDOS-OPS-005.** Один mailbox/portal scope имеет одного владельца observer schedule
и один active runtime instance. Startup обязан проверить revoked V4 writer, durable
outbox quarantine, read-only authority, expiry, write budget zero и lock до первого
IMAP/Bitrix read; generation/TTL и zero-write scope перепроверяются до и после каждого
remote call.
