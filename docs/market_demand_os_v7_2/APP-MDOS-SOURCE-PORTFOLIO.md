...
**MDOS-SRC-005.** IMAP mailbox, нативная почта Bitrix и Bitrix Activity API имеют
раздельные Source Capability Passports с exact tenant/mailbox scope. Для observer
разрешимы только IMAP read-only и `crm.activity.list/get`; writer capability
принадлежит нативной интеграции Bitrix, а SMTP/CRM writes остаются запрещены.
...
**MDOS-PORT-008.** Первичный inbound email входит в Tier A+ через нативную почту
Bitrix как sole inbound CRM writer. Custom observer является отдельным assurance
sensor: он подтверждает projection с 5m stability/90m review, не создаёт lead/task и
не превращает mail observation в контактное разрешение.
